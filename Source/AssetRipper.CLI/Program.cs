...
Arguments? argsParsed = Arguments.Parse(args);
...
if (argsParsed.Log)
{
	string? logPath = argsParsed.LogPath;
...
switch (argsParsed.Mode)
{
	case ExtractMode.UnityProject:
		await GameFileLoader.ExportUnityProject(diOutput.FullName);
		break;
	case ExtractMode.PrimaryContent:
		await GameFileLoader.ExportPrimaryContent(diOutput.FullName);
		break;
}
